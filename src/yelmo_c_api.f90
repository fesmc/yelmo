...
contains
  ...
  subroutine yelmo_get_var2D(v2D, nx, ny, name, alias) bind(C, name="yelmo_get_var2D")
    ...
    select case(trim(f_name))
      ...
      case("bnd_smb_ref");       v2D = real(ylmo%bnd%smb,           c_double)
      ...
      case("bnd_calv_mask");     v2D = merge(1.0_c_double, 0.0_c_double, ylmo%bnd%calv_mask)
  ...
  subroutine yelmo_get_var3D(v3D, nx, ny, nz, name, alias) bind(C, name="yelmo_get_var3D")
    ...
    select case(trim(f_name))
      ...
      case("trc_t_dep");         v3D = real(ylmo%trc%now%t_dep,         c_double)
      case("trc_depth_iso");     v3D = real(ylmo%trc%now%depth_iso,     c_double)
      ! strain_3D_class (mat)
