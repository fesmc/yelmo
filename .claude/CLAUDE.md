...
The `fasthydrology` symlink is expected by every host config in `config/`
(see `FASTHYDROROOT` / `LIB_FASTHYDRO`). `libfasthydro.a` must be built
separately by running `make fasthydro-static` inside the fasthydrology
checkout — yelmo's Makefile does not recurse into it (same convention as
`fesm-utils`).
