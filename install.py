#!/usr/bin/env python3
"""
Interactive installer for Yelmo.

Each dependency has its own install_* function that performs the steps
specific to that repo: clone, configure for the current system, plus any
repo-local extras (e.g. internal symlinks). Yelmo only depends on fesm-utils;
to add a new dependency later, write a new install_<repo>(state) function
...
Run from the yelmo repo root:
    python3 install.py
    python3 install.py -d clone-https        # clone via HTTPS instead of SSH
    python3 install.py -d no                 # fesm-utils already under yelmo; just link
...
def clone_into(state, name, org, dirname):
    ...
    When download == 'no', the canonical path is yelmo_root/<dirname> (which may
    be a real dir or a symlink the user already arranged); otherwise it is
    install_dir/<dirname>."""
    if state.download == "no":
        dest = state.yelmo_root / dirname
    else:
...
def install_runme(state):
    ...
    if not state.do_config_steps:
        return
    log_section(state, "yelmo runme_config")
    src = state.yelmo_root / ".runme" / "runme_config"
    dst = state.yelmo_root / ".runme_config"
    if not src.exists():
    ...
    if dst.exists():
        ...
        log_raw(state, "# .runme_config already present; to regenerate: runme --config", cwd=state.yelmo_root)
        ...
        run(state, ["runme", "--config"], cwd=state.yelmo_root)
    else:
        print(f"  ! `runme` not on PATH — copying {src.name} to {dst.name} directly")
        print(f"  $ cp {src} {dst}")
        dst.write_bytes(src.read_bytes())
        log_raw(state, "# runme not on PATH; equivalent of `runme --config`:", cwd=state.yelmo_root)
        log_raw(state, "cp .runme/runme_config .runme_config", cwd=state.yelmo_root)
    ...
    log_raw(state,
            ...
            cwd=state.yelmo_root)
    log_raw(state,
            'sed -i.bak -E "s/(\\"account\\"[[:space:]]*:[[:space:]]*\\")[^\\"]*(\\")/'
            '\\1${account}\\2/" .runme_config',
            cwd=state.yelmo_root)
    log_raw(state, "rm .runme_config.bak", cwd=state.yelmo_root)
...
def install_fesm_utils(state):
    ...
    # yelmo-side $(FESMUTILSROOT) points at fesm-utils/utils for this reason.
...
def install_yelmo(state):
    """yelmo itself: config + root-level link to fesm-utils. External data
    link (ice_data) is handled separately by setup_data_links."""
    log_section(state, "yelmo (config + root links)")
    do_config(state, "yelmo", state.yelmo_root)

    # Root-level link to fesm-utils. With -d no the repo path *is* the
    # yelmo-root entry, so the link is a no-op; skip it.
    if state.download != "no":
        make_link(state, state.repo_paths["fesm-utils"], state.yelmo_root / "fesm-utils")


def setup_data_links(state):
    """Prompt for ice_data path and create a symlink under yelmo/. May be
    skipped (the summary will remind the user to wire it up later). Run after
    the per-repo installs and before runme_config — this is a local-filesystem
    path, separate from HPC settings."""
    ...
    if state.ice_data_path:
        p = Path(state.ice_data_path).expanduser().resolve()
        if not p.exists():
            print(f"  ! ice_data path {p} does not exist — creating link anyway")
        make_link(state, p, state.yelmo_root / "ice_data", absolute=True)
    else:
        state.data_pending.append("ice_data")


# ---------------------------------------------------------------- orchestration

def load_hpc_names(yelmo_root):
    qf = yelmo_root / ".runme" / "queues.json"
    if not qf.exists():
        return []
    try:
...
def collect_initial_inputs(yelmo_root, download, do_config_steps,
                           default_cfg, overwrite):
    print(f"Yelmo root: {yelmo_root}")
    ...
    if download == "no":
        print("  1. Use fesm-utils already present under yelmo/")
    else:
        print(f"  1. Clone the fesm-utils dependency repository ({download})")
    if do_config_steps:
    ...
    if download == "no":
        install_dir = yelmo_root  # not really used; fesm-utils lives under yelmo/
    else:
        install_dir = Path(ask("Directory to clone fesm-utils into",
                               default=str(Path.cwd()))).expanduser().resolve()
        install_dir.mkdir(parents=True, exist_ok=True)

    state = State(
        yelmo_root=yelmo_root,
    ...
    # ice_data_src var is registered later from inside setup_data_links, after
    # the user has provided the path.
    register_path_var(state, "yelmo_root", yelmo_root)
    if download != "no":
...
def collect_runme_inputs(state):
    ...
    hpcs = load_hpc_names(state.yelmo_root)
    if hpcs:
...
def write_install_sh(state):
    path = state.yelmo_root / ".install.sh"
    out = [
        "#!/usr/bin/env bash",
        "# Generated by install.py.",
        "# Reproduces a Yelmo install with the choices made in the interactive run.",
...
def print_summary(state):
    ...
    print(f"  {state.yelmo_root / '.install.sh'}")
    ...
    print(f"     If you skip this, `make benchmarks` will abort early with a")
    print(f"     LIS/FFTW-not-built error (no half-built state).")
    print(f"  2. Compile yelmo (builds fesm-utils too):")
    print(f"       cd {state.yelmo_root}")
    print(f"       make clean")
    print(f"       make benchmarks      # default build")
...
def main():
    parser = argparse.ArgumentParser(description="Interactive Yelmo installer.")
    parser.add_argument("-d", "--download",
                        choices=["clone-ssh", "clone-https", "no", "none"],
                        default="clone-ssh",
                        help="How to obtain the fesm-utils repo. "
                             "'clone-ssh' (default) and 'clone-https' do a git clone. "
                             "'no'/'none' assumes fesm-utils is already available at "
                             "yelmo/fesm-utils (as a dir or symlink) and only sets up "
    ...
    parser.add_argument("--overwrite",
                        action="store_true",
                        help="Re-clone fesm-utils. Any existing copy "
    ...
    yelmo_root = Path(__file__).resolve().parent
    state = collect_initial_inputs(yelmo_root, download,
                                   do_config_steps=not args.no_config,
                                   default_cfg=args.config_default,
                                   overwrite=args.overwrite)

    if state.download == "no":
        print(f"\n=== Using fesm-utils already under {state.yelmo_root} ===")
    else:
        print(f"\n=== Installing fesm-utils into {state.install_dir} ===")
