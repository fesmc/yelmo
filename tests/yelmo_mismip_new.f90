...
program yelmo_mismip
    ...
    select case(trim(experiment))
        ...
        case("linear")
            ...
            where(yelmo1%bnd%z_bed .lt. -1000.0) yelmo1%bnd%mask_ice = MASK_ICE_NONE
...
!             yelmo1%tpo%now%H_ice = max(0.0, 10.0 + (300.0-100.0)*(750.0-sqrt((yelmo1%grd%x*1e-3)**2+(yelmo1%grd%y*1e-3)**2))/750.0)
    ...
    ! 1D file 
    call yelmo_write_reg_init(yelmo1,file1D,time_init=time_init,units="years",mask=(yelmo1%bnd%mask_ice /= MASK_ICE_NONE))
